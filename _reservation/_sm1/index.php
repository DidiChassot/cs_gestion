...
/****************************************************************************************************************************************/
	...
	$titre_event = $dataEventAtt['titre'];
      } else {
...
if($zone=='event' || $zone=='reserv' ) {
?>

<hr class="clear" name="liste" id="liste">

<h2><?php echo utf8_encode($titre_event); ?></h2>
<div class="left">
...
if($add == 1) {
  echo '<h4>Ajout</h4>';
  echo '<form action="'.$_SERVER['PHP_SELF'].'?add=0&amp;zone=reserv&amp;event='.$event.'&amp;reserv='.$reserv.'&amp;cat='.$cat.'#liste" method="post">';
    ?>
    <div>
...
?>
<h4>Modification</h4>
<form action="<?php $_SERVER['PHP_SELF']; ?>?add=0&amp;zone=reserv&amp;event=<?php echo $event; ?>&amp;reserv=<?php echo $reserv; ?>&amp;cat=<?php echo $cat; ?>#liste" method="post">
  <div>
    <label for="prenom">Prénom</label>
...
/**********************************************************************************************************************************************/
    ...
    while ($dataInscripAtt = mysqli_fetch_array($resultInscripAtt)) {
      echo '<tr>';
	//Contact
	echo '<td><a href="'.$_SERVER['PHP_SELF'].'?zone=reserv&amp;mod=1&amp;event='.$event.'&amp;reserv='.$reserv.'&cat='.$cat.'&amp;inscrip='.$dataInscripAtt['id'].'#liste">'.utf8_encode($dataInscripAtt['contact']).'</a></td>';
	...
	if($_COOKIE['role']=='e') {	
	  echo '<td>';
	    echo '<form name="suppr_news" action="'.$_SERVER['PHP_SELF'].'?zone=reserv&amp;event='.$event.'&amp;reserv='.$reserv.'&amp;cat='.$cat.'#liste" method="post" onclick="return confirm(\'voulez-vous vraiment supprimer cette inscription?\')">
