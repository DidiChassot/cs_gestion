...
$start_base = $start_bulletin;
$end_base = $end_bulletin;
...
$sql_allday = "SELECT * FROM pm_allday WHERE (start <= '$end_bulletin'AND start >= '$start_bulletin') OR (end >= '$start_bulletin' AND end <= '$end_bulletin') AND actif = 'a' ORDER BY start";
...
/** EVENTS VIDES
 *
 * Création des "events vides" afin d'éviter le décalage des séances pour la programmation
 */
$start_bulletin = strtotime($start_bulletin);
$end_bulletin = strtotime($end_bulletin);

$startWeek = strtotime("Monday", $start_bulletin);
$endWeek = strtotime("Sunday", $end_bulletin);

if($startWeek != $start_bulletin) { $startWeek =  strtotime("- 1 week", $startWeek ); }


$weekArray = []; 
for ($start_day = $startWeek; $start_day <= $endWeek; $start_day = strtotime("+ 1 day", $start_day )) {
    $sqlCountEvent = "SELECT COUNT(id) AS numevent FROM pm_allday WHERE start <= FROM_UNIXTIME('$start_day') AND end >= FROM_UNIXTIME('$start_day') AND actif = 'a' ORDER BY start";
    $resultCountEvent = mysqli_query($connexion, $sqlCountEvent ) or die(mysqli_error());
    $dataCountEvent = mysqli_fetch_array($resultCountEvent);
    
    if($weekArray[date('W', $start_day)] < $dataCountEvent['numevent']) $weekArray[date('W', $start_day)] = $dataCountEvent['numevent'];
}
...
for ($start_day = $startWeek; $start_day <= $endWeek; $start_day = strtotime("+ 1 day", $start_day )) {
    ...
    $resultCountEvent = mysqli_query($connexion, $sqlCountEvent ) or die(mysqli_error());
    $dataCountEvent = mysqli_fetch_array($resultCountEvent);
//echo 'test: '.date('Y-m-d', $endWeek).'/'.$dataCountEvent['numevent'].'<br>';
    $numEvent = $dataCountEvent['numevent'];
    while ($numEvent < $weekArray[date('W', $start_day)]){
	$numEvent++;
...
$start_bulletin = $start_base;
$end_bulletin = $end_base;
