...
if($calendar=='bulletin') {
    $sql = "SELECT * FROM pm_seance WHERE actif = 'a' AND start > '$start_bulletin' AND start <= '$end_bulletin' ORDER BY id_seance";
    ...
    while ($data = mysqli_fetch_array($result)) {
        ...
        if($data['titre']) {
            ...
            $titre_affiche = str_replace($caract, $new_caract, $data["titre"]);
            ...
            $titre_affiche = str_replace($caract, $new_caract, $data["titre"]);
            // titre de la séance
            $titre = $pos.' '.html_entity_decode(utf8_encode($titre_affiche));
            ...
            // titre de la séance
            $titre = $pos.' '.utf8_encode($titre_affiche);
            if ($data['commentaire']) {$titre .= '<br />'.utf8_encode($data['commentaire']);}
        }
    ...
    $sql = "SELECT * FROM pm_seance, pm_film_seance
    WHERE pm_seance.actif = 'a'
    AND pm_seance.start > '$start_bulletin'
    AND pm_seance.start <= '$end_bulletin'
    AND pm_seance.id_seance = pm_film_seance.id_seance
    AND pm_film_seance.id_film = '$id_film'
    ORDER BY pm_seance.id_seance";
    $result = mysqli_query($connexion, $sql) or die(mysqli_error());
    while ($data = mysqli_fetch_array($result)) {
        // sélection des informations du (1er) film   
        $sql_movie = "SELECT * FROM pm_film_seance, pm_film, pm_cycle
