...
<link href='<?php echo $_COOKIE['indexApp']."/".$_COOKIE['sousMenu']."/"; ?>css/fullcalendar.print.css' rel='stylesheet' media='print' />
...
<script src='<?php echo $_COOKIE['indexApp']."/".$_COOKIE['sousMenu']."/"; ?>js/fullcalendar.js'></script>
...
if ($action=='clic'){ // si on clic sur une séance
	...
	$data_plage = mysqli_fetch_array($result_plage);
		$start = $data_plage['start'];
		$defaultDate = $start; //date du bulletin sélectionné
		$defaultDate2 = date("Y-m-d",strtotime($defaultDate. " +1 month")); //date du bulletin sélectionné
